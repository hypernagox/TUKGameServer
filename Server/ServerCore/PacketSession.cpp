...
{
	PacketSession::PacketSession(S_ptr<ContentsEntity> pEntity_, const PacketHandleFunc* const sessionPacketHandler_)noexcept
		:Session{ std::move(pEntity_),sessionPacketHandler_ }
	{
	}

	PacketSession::PacketSession(S_ptr<ContentsEntity> pEntity_, const PacketHandleFunc* const sessionPacketHandler_, const SOCKET sock) noexcept
		:Session{ std::move(pEntity_), sessionPacketHandler_ }
	{
	}
