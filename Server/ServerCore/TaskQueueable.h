...
namespace ServerCore
{
	...
	public:
		...
		{
			EnqueueAsyncTask(Task(memFunc, this->SharedCastThis<T>(), std::forward<Args>(args)...));
		}
		template<typename T, typename U, typename Ret, typename... Args> requires std::derived_from<U,T>
		...
		{
			Mgr(TaskTimerMgr)->ReserveAsyncTask(tickAfter, shared_from_this(), Task(std::forward<Func>(fp), std::forward<Args>(args)...));
		}
		template<typename T, typename Ret, typename... Args> requires std::derived_from<T, TaskQueueable>
