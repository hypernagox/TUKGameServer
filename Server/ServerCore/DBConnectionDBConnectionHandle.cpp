...
namespace ServerCore
{
	...
	const bool DBConnectionHandle::BindParam(c_int32 paramIndex, const WCHAR* const str, SQLLEN* const index)const noexcept
	{
		const SQLULEN size = static_cast<const SQLULEN>((::wcslen(str) + 1) * 2);
		*index = SQL_NTSL;
	...
	//const bool DBConnectionHandle::BindParam(c_int32 paramIndex, BYTE* const bin, int32 size, SQLLEN* const index)const noexcept
	//{
	//	if (bin == nullptr)
	//	{
	//		*index = SQL_NULL_DATA;
	...
	//const bool DBConnectionHandle::BindCol(c_int32 columnIndex, BYTE* const bin, c_int32 size, SQLLEN* const index)const noexcept
	//{
