...
namespace ServerCore
{
	...
	public:
		Session(S_ptr<ContentsEntity> pEntity_,const PacketHandleFunc* const sessionPacketHandler_)noexcept;
		Session(S_ptr<ContentsEntity> pEntity_,const PacketHandleFunc* const sessionPacketHandler_,const SOCKET sock)noexcept;
		virtual ~Session();
	...
	public:
		...
		bool Connect();
		bool Disconnect(std::wstring_view cause)noexcept;
	...
	public:
		const auto operator<=>(const Session& other)const noexcept { return GetObjectID() <=> other.GetObjectID(); }
		...
		const ID_Ptr<SessionManageable> ExchangeSessionRoomInfo(const uint16_t roomID_, const SessionManageable* const pSessionRoom_, const std::memory_order m_order = std::memory_order_relaxed)noexcept {
			return m_CurrentSessionRoomInfo.exchange(ID_Ptr<SessionManageable>{ roomID_, pSessionRoom_ }, m_order);
		}
	private:
